import { lazy } from 'react';
import { Routes } from '@/shared/constants';
import { IRoute, RolesDict } from '@/shared/types';

export const routes: IRoute[] = [
  {
    id: 0,
    path: '/',
    component: lazy(() => import('../ui/auth')),
    isPublic: true,
    roles: [RolesDict.APPLICANT, RolesDict.EMPLOYEE, RolesDict.STUDENT],
    withLayout: false,
  },
  {
    id: 1,
    path: Routes.MAIN,
    component: lazy(() => import('../ui/main')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 2,
    path: Routes.CERTIFICATE,
    component: lazy(() => import('../ui/certificates')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 3,
    path: Routes.CONSULTATION,
    component: lazy(() => import('../ui/consultations')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 4,
    path: Routes.DISTRICT,
    component: lazy(() => import('../ui/dismissal')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 5,
    path: Routes.EMPLOYMENT_CONTRACT,
    component: lazy(() => import('../ui/employment-contract')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 6,
    path: Routes.INTERNAL_ADMISSION,
    component: lazy(() => import('../ui/internal-admission')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 7,
    path: Routes.TRANSFER,
    component: lazy(() => import('../ui/transfer')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 8,
    path: Routes.VACATION,
    component: lazy(() => import('../ui/vacation')),
    isPublic: false,
    roles: [RolesDict.EMPLOYEE],
    withLayout: true,
  },
  {
    id: 9,
    path: Routes.REQUESTS,
    component: lazy(() => import('../ui/type-of-requests')),
    isPublic: false,
    roles: [RolesDict.APPLICANT, RolesDict.EMPLOYEE, RolesDict.STUDENT],
    withLayout: false,
  },
];
